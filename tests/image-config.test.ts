...
describe("matchRemotePattern hostname", () => {
  it("matches exact hostname", () => {
    ...
    expect(matchRemotePattern(pattern, new URL("https://example.com/img.png"))).toBe(
      true,
    );
  });
  ...
  it("matches single-segment wildcard (*)", () => {
    const pattern: RemotePattern = { hostname: "*.example.com" };
    expect(matchRemotePattern(pattern, new URL("https://cdn.example.com/img.png"))).toBe(
      true,
    );
    expect(
      matchRemotePattern(pattern, new URL("https://images.example.com/img.png")),
    ).toBe(true);
  });
  ...
  it("single wildcard does not match deep subdomains", () => {
    const pattern: RemotePattern = { hostname: "*.example.com" };
    expect(
      matchRemotePattern(pattern, new URL("https://deep.cdn.example.com/img.png")),
    ).toBe(false);
  });
  ...
  it("matches double-star wildcard (**) for deep subdomains", () => {
    ...
    expect(
      matchRemotePattern(pattern, new URL("https://a.b.c.example.com/img.png")),
    ).toBe(true);
  });
...
describe("matchRemotePattern protocol", () => {
  ...
  it("rejects when protocol doesn't match", () => {
    const pattern: RemotePattern = { hostname: "example.com", protocol: "https" };
    expect(matchRemotePattern(pattern, new URL("http://example.com/img.png"))).toBe(
      false,
    );
  });
...
describe("matchRemotePattern port", () => {
  it("matches specific port", () => {
    ...
    expect(matchRemotePattern(pattern, new URL("https://example.com:8080/img.png"))).toBe(
      true,
    );
  });
  ...
  it("rejects wrong port", () => {
    const pattern: RemotePattern = { hostname: "example.com", port: "8080" };
    expect(matchRemotePattern(pattern, new URL("https://example.com:3000/img.png"))).toBe(
      false,
    );
  });
  ...
  it("skips port check when not specified", () => {
    const pattern: RemotePattern = { hostname: "example.com" };
    expect(matchRemotePattern(pattern, new URL("https://example.com:9999/img.png"))).toBe(
      true,
    );
  });
});
...
describe("matchRemotePattern pathname", () => {
  it("defaults to ** (match everything) when not specified", () => {
    const pattern: RemotePattern = { hostname: "example.com" };
    expect(
      matchRemotePattern(pattern, new URL("https://example.com/any/path/here.png")),
    ).toBe(true);
  });
  ...
  it("matches exact pathname", () => {
    ...
    expect(
      matchRemotePattern(pattern, new URL("https://example.com/images/hero.png")),
    ).toBe(true);
    expect(
      matchRemotePattern(pattern, new URL("https://example.com/images/other.png")),
    ).toBe(false);
  });
  ...
  it("matches single-segment pathname wildcard", () => {
    ...
    expect(
      matchRemotePattern(pattern, new URL("https://example.com/images/photo.png")),
    ).toBe(true);
    expect(
      matchRemotePattern(pattern, new URL("https://example.com/images/deep/photo.png")),
    ).toBe(false);
  });
  ...
  it("matches multi-segment pathname wildcard", () => {
    ...
    expect(
      matchRemotePattern(pattern, new URL("https://example.com/images/a/b/c.png")),
    ).toBe(true);
  });
...
describe("matchRemotePattern search", () => {
  it("matches exact search string", () => {
    ...
    expect(matchRemotePattern(pattern, new URL("https://example.com/img.png?v=1"))).toBe(
      true,
    );
  });
  ...
  it("rejects wrong search string", () => {
    const pattern: RemotePattern = { hostname: "example.com", search: "?v=1" };
    expect(matchRemotePattern(pattern, new URL("https://example.com/img.png?v=2"))).toBe(
      false,
    );
  });
  ...
  it("skips search check when not specified", () => {
    const pattern: RemotePattern = { hostname: "example.com" };
    expect(
      matchRemotePattern(pattern, new URL("https://example.com/img.png?anything=here")),
    ).toBe(true);
  });
});
...
describe("hasRemoteMatch", () => {
  ...
  it("does not match unrecognized domain", () => {
    expect(
      hasRemoteMatch(["cdn.example.com"], [], new URL("https://evil.com/photo.png")),
    ).toBe(false);
  });
...
describe("matchRemotePattern glob edge cases", () => {
  it("escapes regex special characters in hostname", () => {
    ...
    expect(
      matchRemotePattern(pattern, new URL("https://my.cdn.example.com/img.png")),
    ).toBe(true);
    // "myXcdnXexample.com" should NOT match (dot is literal, not regex any-char)
    expect(
      matchRemotePattern(pattern, new URL("https://myXcdnXexampleXcom/img.png")),
    ).toBe(false);
  });
  ...
  it("escapes regex special characters in pathname", () => {
    ...
    expect(
      matchRemotePattern(pattern, new URL("https://example.com/images/photoXpng")),
    ).toBe(false);
  });

  it("handles multiple wildcards in pattern", () => {
    const pattern: RemotePattern = { hostname: "*.*.example.com" };
    expect(matchRemotePattern(pattern, new URL("https://a.b.example.com/img.png"))).toBe(
      true,
    );
    expect(matchRemotePattern(pattern, new URL("https://a.example.com/img.png"))).toBe(
      false,
    );
  });
  ...
  it("combines hostname and pathname globs", () => {
    ...
    expect(
      matchRemotePattern(pattern, new URL("https://cdn.example.com/uploads/a/b.png")),
    ).toBe(true);
    expect(
      matchRemotePattern(pattern, new URL("https://cdn.example.com/other/a.png")),
    ).toBe(false);
    expect(
      matchRemotePattern(pattern, new URL("http://cdn.example.com/uploads/a.png")),
    ).toBe(false);
  });
});
