...
/** Run the og-font-patch transform on a fake edge entry and return the result */
function transformEdgeEntry(
  idPath: string,
  yogaBase64 = FAKE_YOGA_B64,
...
describe("vinext:og-font-patch plugin", () => {
  ...
  describe("yoga WASM transform", () => {
    ...
    it("uses dynamic import for yoga WASM with catch fallback", () => {
      const { code } = transformEdgeEntry(path.join(fakeOgDistDir, "index.edge.js"));
      expect(code).toContain('import("./yoga.wasm?module")');
      expect(code).toContain(".catch(");
    });

    it("includes inline base64 bytes as Node.js fallback for yoga", () => {
      const { code } = transformEdgeEntry(path.join(fakeOgDistDir, "index.edge.js"));
    ...
    it("writes yoga.wasm to disk at transform time", () => {
      ...
      transformEdgeEntry(path.join(writeDistDir, "index.edge.js"));
  ...
  describe("Node.js compatibility invariant", () => {
