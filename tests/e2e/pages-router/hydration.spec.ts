...
test.describe("Hydration", () => {
  ...
  test("SSR HTML is present before hydration (no flash)", async ({ page }) => {
    ...
    await expect(page.locator("h1")).toHaveText("Hello, nextcompat!");
    await expect(page.locator("p")).toContainText(
      "This is a Pages Router app running on Vite.",
    );
  });

  test("_app.tsx wrapper persists across client navigations", async ({ page }) => {
    await page.goto(`${BASE}/`);
  ...
  test("state is preserved within client navigation", async ({ page }) => {
  ...
  test("__NEXT_DATA__ is present in the page and contains route info", async ({ page }) => {
