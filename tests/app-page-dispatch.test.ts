...
import { probeReactServerSubtree } from "../packages/vinext/src/server/app-page-probe.js";
import {
...
const paramAccessCases: readonly [string, LayoutParamProbeReader][] = [
  ["direct params.slug access", (params) => Reflect.get(Object(params), "slug")],
  [
    "await params access",
    async (params) => {
      await params;
  ...
  [
    "destructured params access",
    (params) => {
      const { slug } = Object(params) as { slug?: string };
      return slug;
    },
  ],
];
...
function createCacheTagLayoutProbe(
  route: TestRoute,
  matchedParams: Record<string, string | string[]>,
  cacheTag: string,
  options: { wrapDataChild?: "forwardRef" | "lazy" | "memo" } = {},
): DispatchOptions["probeLayoutAt"] {
  return (layoutIndex, layoutParamAccess) => {
    const treePath = createAppPageTreePath(
      route.routeSegments,
    ...
    const runProbe = (tracker: AppLayoutParamAccessTracker | undefined) => {
      ...
      function DataChild() {
        getRequestContext().currentRequestTags.push(cacheTag);
        return null;
      }
      const RenderedDataChild =
        options.wrapDataChild === "memo"
          ? React.memo(DataChild)
          : options.wrapDataChild === "forwardRef"
            ? React.forwardRef(function ForwardRefDataChild() {
                return React.createElement(DataChild);
              })
            : options.wrapDataChild === "lazy"
              ? React.lazy(() => Promise.resolve({ default: DataChild }))
              : DataChild;
      function Layout() {
        return React.createElement("section", null, React.createElement(RenderedDataChild));
      }
      return probeReactServerSubtree(React.createElement(Layout));
    };
...
function createRequestApiLayoutProbe(
  route: TestRoute,
  matchedParams: Record<string, string | string[]>,
): DispatchOptions["probeLayoutAt"] {
  return (layoutIndex, layoutParamAccess) => {
...
function createUnstableCacheLayoutProbe(
  route: TestRoute,
  readCachedLayoutData: () => Promise<unknown>,
): DispatchOptions["probeLayoutAt"] {
  return (layoutIndex, layoutParamAccess) => {
    const treePath = createAppPageTreePath(
      route.routeSegments,
...
describe("app page dispatch", () => {
  ...
  it.each(paramAccessCases)(
    "does not skip a static dynamic-segment layout after %s",
    async (_name, readParams) => {
      const originalBuildId = process.env.__VINEXT_BUILD_ID;
      process.env.__VINEXT_BUILD_ID = "deploy-test";
      const sourceRouteId = "route:/blog/hello-world";
      const targetRouteId = "route:/blog/getting-started";
      const routePattern = "/blog/[slug]";
      const rootBoundaryId = "/";
      const layoutId = AppElementsWire.encodeLayoutId("/blog/[slug]");
      const pageId = AppElementsWire.encodePageId("/blog/getting-started", null);
      ...
      try {
        const { options } = createDispatchOptions({
          buildPageElement: async () => element,
          cleanPathname: "/blog/getting-started",
          clientReuseManifest,
          isProduction: true,
          isRscRequest: true,
          params,
          probeLayoutAt: createLayoutParamProbe(route, params, [readParams]),
          renderToReadableStream(payload) {
        ...
        expect(capturedPayloads[0][layoutId]).toBe("blog-slug-layout");
        expect(capturedPayloads[0][pageId]).toBe("post-page");
      } finally {
        if (originalBuildId === undefined) {
          delete process.env.__VINEXT_BUILD_ID;
        } else {
  ...
  it("does not skip a static dynamic-segment layout with unobserved non-empty params", async () => {
    const originalBuildId = process.env.__VINEXT_BUILD_ID;
    process.env.__VINEXT_BUILD_ID = "deploy-test";
    const sourceRouteId = "route:/acme/settings";
    const targetRouteId = "route:/globex/settings";
    const routePattern = "/[team]/settings";
    const rootBoundaryId = "/";
    const layoutId = AppElementsWire.encodeLayoutId("/[team]");
    const pageId = AppElementsWire.encodePageId("/globex/settings", null);
    const params = { team: "globex" };
    const route = createRoute({
      __buildTimeClassifications: new Map([[0, "static"]]),
    ...
    try {
      const { options } = createDispatchOptions({
        buildPageElement: async () => element,
        cleanPathname: "/globex/settings",
        clientReuseManifest,
        isProduction: true,
        isRscRequest: true,
        params,
        probeLayoutAt: createLayoutParamProbe(route, params, [null]),
        renderToReadableStream(payload) {
          capturedPayloads.push(captureRecord(payload));
          return createStream(["flight"]);
        },
      ...
      expect(response.status).toBe(200);
      expect(capturedPayloads).toHaveLength(1);
      expect(capturedPayloads[0][layoutId]).toBe("team-layout");
      expect(capturedPayloads[0][pageId]).toBe("settings-page");
    } finally {
      if (originalBuildId === undefined) {
        delete process.env.__VINEXT_BUILD_ID;
      } else {
  ...
  it("does not skip a static layout whose probe observes cache-tagged data", async () => {
    const originalBuildId = process.env.__VINEXT_BUILD_ID;
    process.env.__VINEXT_BUILD_ID = "deploy-test";
    const routeId = "route:/dashboard/profile";
    ...
    const route = createRoute({
      ...
      layouts: [{ default() {} }],
      pattern: routePattern,
      routeSegments: ["dashboard", "profile"],
    });
    const clientReuseManifest = createVerifiedStaticLayoutManifest({
      deploymentVersion: "deploy-test",
      layoutId,
    ...
    try {
      const { options } = createDispatchOptions({
        ...
        isRscRequest: true,
        params: {},
        probeLayoutAt: createCacheTagLayoutProbe(route, {}, "tag:dashboard-layout"),
        renderToReadableStream(payload) {
          capturedPayloads.push(captureRecord(payload));
          return createStream(["flight"]);
        },
      ...
      expect(response.status).toBe(200);
      expect(capturedPayloads).toHaveLength(1);
      expect(capturedPayloads[0][layoutId]).toBe("dashboard-layout");
      expect(capturedPayloads[0][pageId]).toBe("profile-page");
    } finally {
      if (originalBuildId === undefined) {
        delete process.env.__VINEXT_BUILD_ID;
      } else {
  ...
  it("does not skip a static layout whose probe observes request APIs", async () => {
    const originalBuildId = process.env.__VINEXT_BUILD_ID;
    process.env.__VINEXT_BUILD_ID = "deploy-test";
    const routeId = "route:/dashboard/profile";
    const routePattern = "/dashboard/profile";
    ...
    try {
      const { options } = createDispatchOptions({
        ...
        isRscRequest: true,
        params: {},
        probeLayoutAt: createRequestApiLayoutProbe(route, {}),
        renderToReadableStream(payload) {
          capturedPayloads.push(captureRecord(payload));
          return createStream(["flight"]);
        },
      ...
      expect(response.status).toBe(200);
      expect(capturedPayloads).toHaveLength(1);
      expect(capturedPayloads[0][layoutId]).toBe("tenant-layout");
      expect(capturedPayloads[0][pageId]).toBe("profile-page");
      expect(capturedPayloads[0][APP_LAYOUT_FLAGS_KEY]).toEqual({
        [layoutId]: "d",
      });
    } finally {
      if (originalBuildId === undefined) {
  ...
  it("does not skip a static layout whose probe observes unstable_cache data", async () => {
    ...
    const element = {
      [APP_ROUTE_KEY]: routeId,
      [APP_ROOT_LAYOUT_KEY]: rootBoundaryId,
      [layoutId]: "banner-layout",
      [pageId]: "profile-page",
    };
    let calls = 0;
    const readCachedLayoutData = unstable_cache(
      async () => {
        calls += 1;
        return `banner-${calls}`;
      },
      ["dashboard-banner"],
      { tags: ["dashboard-banner"], revalidate: 60 },
    );
    await runWithRequestContext(createRequestContext(), () => readCachedLayoutData());
    const capturedPayloads: Record<string, unknown>[] = [];
    ...
    try {
      const { options } = createDispatchOptions({
        ...
        isRscRequest: true,
        params: {},
        probeLayoutAt: createUnstableCacheLayoutProbe(route, readCachedLayoutData),
        renderToReadableStream(payload) {
          capturedPayloads.push(captureRecord(payload));
          return createStream(["flight"]);
        },
      ...
      expect(response.status).toBe(200);
      expect(capturedPayloads).toHaveLength(1);
      expect(capturedPayloads[0][layoutId]).toBe("banner-layout");
      expect(capturedPayloads[0][pageId]).toBe("profile-page");
      expect(capturedPayloads[0][APP_LAYOUT_FLAGS_KEY]).toEqual({
        [layoutId]: "d",
      });
  ...
  it("marks runtime-probed request-api layouts dynamic in payload metadata", async () => {
    const routeId = "route:/dashboard/profile";
    const routePattern = "/dashboard/profile";
    const rootBoundaryId = "/";
    const layoutId = AppElementsWire.encodeLayoutId("/");
  ...
  it.each(["memo", "forwardRef", "lazy"] as const)(
    "does not skip a static layout whose %s child observes cache-tagged data",
    async (wrapDataChild) => {
      const originalBuildId = process.env.__VINEXT_BUILD_ID;
      process.env.__VINEXT_BUILD_ID = "deploy-test";
      const routeId = "route:/dashboard/profile";
  ...
  it("does not skip a static optional-catchall layout when the target params are empty", async () => {
    const originalBuildId = process.env.__VINEXT_BUILD_ID;
    process.env.__VINEXT_BUILD_ID = "deploy-test";
    const sourceRouteId = "route:/docs/a";
    const targetRouteId = "route:/docs";
    const routePattern = "/docs/[[...slug]]";
    const rootBoundaryId = "/";
    const layoutId = AppElementsWire.encodeLayoutId("/docs/[[...slug]]");
    const pageId = AppElementsWire.encodePageId("/docs", null);
    const params = { slug: [] };
    const route = createRoute({
      __buildTimeClassifications: new Map([[0, "static"]]),
      layoutTreePositions: [2],
    ...
    const element = {
      [APP_ROUTE_KEY]: targetRouteId,
      [APP_ROOT_LAYOUT_KEY]: rootBoundaryId,
      [layoutId]: "docs-index-layout",
      [pageId]: "docs-index-page",
    };
    const capturedPayloads: Record<string, unknown>[] = [];
    ...
    try {
      ...
      expect(response.status).toBe(200);
      expect(capturedPayloads).toHaveLength(1);
      expect(capturedPayloads[0][layoutId]).toBe("docs-index-layout");
      expect(capturedPayloads[0][pageId]).toBe("docs-index-page");
    } finally {
      if (originalBuildId === undefined) {
        delete process.env.__VINEXT_BUILD_ID;
      } else {
  ...
  it("keeps param scope isolated across nested static layouts and sibling dynamic values", async () => {
    const originalBuildId = process.env.__VINEXT_BUILD_ID;
    process.env.__VINEXT_BUILD_ID = "deploy-test";
    const sourceRouteId = "route:/acme/settings";
    const targetRouteId = "route:/globex/settings";
    ...
    const teamLayoutId = AppElementsWire.encodeLayoutId("/[team]");
    const settingsLayoutId = AppElementsWire.encodeLayoutId("/[team]/settings");
    const pageId = AppElementsWire.encodePageId("/globex/settings", null);
    const params = { team: "globex" };
    const route = createRoute({
      __buildTimeClassifications: new Map([
        [0, "static"],
        [1, "static"],
        [2, "static"],
      ]),
      layoutTreePositions: [0, 1, 2],
      layouts: [{ default() {} }, { default() {} }, { default() {} }],
      params: ["team"],
      pattern: routePattern,
      routeSegments: ["[team]", "settings"],
    });
    const clientReuseManifest = createVerifiedStaticLayoutManifest({
      deploymentVersion: "deploy-test",
      layoutIds: [rootLayoutId, teamLayoutId, settingsLayoutId],
      rootBoundaryId,
      routeId: sourceRouteId,
      routePattern,
    });
    const element = {
      [APP_ROUTE_KEY]: targetRouteId,
      [APP_ROOT_LAYOUT_KEY]: rootBoundaryId,
      [rootLayoutId]: "root-layout",
      [teamLayoutId]: "team-layout",
      [settingsLayoutId]: "settings-layout",
      [pageId]: "settings-page",
    };
    const capturedPayloads: Record<string, unknown>[] = [];
    const waitUntilPromises: Promise<unknown>[] = [];
    ...
    try {
      const { options } = createDispatchOptions({
        ...
        isRscRequest: true,
        params,
        probeLayoutAt: createLayoutParamProbe(route, params, [
          null,
          (layoutParams) => Reflect.get(Object(layoutParams), "team"),
          null,
        ]),
        renderToReadableStream(payload) {
          capturedPayloads.push(captureRecord(payload));
      ...
      expect(waitUntilPromises).toHaveLength(0);
      expect(capturedPayloads).toHaveLength(1);
      expect(Object.hasOwn(capturedPayloads[0], rootLayoutId)).toBe(false);
      expect(capturedPayloads[0][teamLayoutId]).toBe("team-layout");
      expect(capturedPayloads[0][settingsLayoutId]).toBe("settings-layout");
      expect(capturedPayloads[0][pageId]).toBe("settings-page");
    } finally {
      if (originalBuildId === undefined) {
        delete process.env.__VINEXT_BUILD_ID;
  ...
  it("falls back to render-and-send when static-layout param observation is incomplete", async () => {
    const originalBuildId = process.env.__VINEXT_BUILD_ID;
    process.env.__VINEXT_BUILD_ID = "deploy-test";
    const routeId = "route:/dashboard/profile";
    const routePattern = "/dashboard/profile";
    ...
    try {
      ...
      expect(response.status).toBe(200);
      expect(capturedPayloads).toHaveLength(1);
      expect(capturedPayloads[0][layoutId]).toBe("root-layout");
      expect(capturedPayloads[0][pageId]).toBe("profile-page");
    } finally {
      if (originalBuildId === undefined) {
        delete process.env.__VINEXT_BUILD_ID;
  ...
  it("does not skip layouts marked dynamic by runtime segment config", async () => {
    const originalBuildId = process.env.__VINEXT_BUILD_ID;
    process.env.__VINEXT_BUILD_ID = "deploy-test";
    const routeId = "route:/posts/hello";
    const routePattern = "/posts/[slug]";
    const rootBoundaryId = "/";
    const layoutId = AppElementsWire.encodeLayoutId("/");
    const pageId = AppElementsWire.encodePageId("/posts/hello", null);
    const element = {
      [APP_ROUTE_KEY]: routeId,
      [APP_ROOT_LAYOUT_KEY]: rootBoundaryId,
      [layoutId]: "root-layout",
    ...
    const route = createRoute({
      layoutTreePositions: [0],
      layouts: [{ default() {}, dynamic: "force-dynamic" }],
      pattern: routePattern,
    });
    const clientReuseManifest = createVerifiedStaticLayoutManifest({
      deploymentVersion: "deploy-test",
  ...
  it("does not skip static layouts with finite revalidate segment config", async () => {
    const originalBuildId = process.env.__VINEXT_BUILD_ID;
    process.env.__VINEXT_BUILD_ID = "deploy-test";
    const routeId = "route:/dashboard/profile";
    const routePattern = "/dashboard/profile";
    ...
    const element = {
      [APP_ROUTE_KEY]: routeId,
      [APP_ROOT_LAYOUT_KEY]: rootBoundaryId,
      [layoutId]: "revalidating-root-layout",
      [pageId]: "profile-page",
    };
    const route = createRoute({
      __buildTimeClassifications: new Map([[0, "static"]]),
      layoutTreePositions: [0],
      layouts: [{ default() {}, revalidate: 60 }],
      pattern: routePattern,
    });
    const clientReuseManifest = createVerifiedStaticLayoutManifest({
      deploymentVersion: "deploy-test",
    ...
    try {
      const { options } = createDispatchOptions({
        ...
        isProduction: true,
        isRscRequest: true,
        probeLayoutAt: createLayoutParamProbe(route, {}, [null]),
        renderToReadableStream(payload) {
          capturedPayloads.push(captureRecord(payload));
          return createStream(["flight"]);
        },
      ...
      expect(response.status).toBe(200);
      expect(capturedPayloads).toHaveLength(1);
      expect(capturedPayloads[0][layoutId]).toBe("revalidating-root-layout");
      expect(capturedPayloads[0][pageId]).toBe("profile-page");
    } finally {
      if (originalBuildId === undefined) {
        delete process.env.__VINEXT_BUILD_ID;
