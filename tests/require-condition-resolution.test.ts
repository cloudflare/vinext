...
function createTransform(resolve: ReturnType<typeof vi.fn>) {
  const hook = createRequireConditionResolutionPlugin().transform;
  const handler = typeof hook === "function" ? hook : hook?.handler;
  return handler!.bind({ resolve } as never) as (
    code: string,
    id: string,
...
describe("vinext:require-condition-resolution", () => {
  it("pre-resolves package require calls with the require import kind", async () => {
    const resolve = vi.fn(
      async (_specifier: string, _importer: string, options: { kind: string }) => ({
        id:
          options.kind === "require-call"
            ? "/app/node_modules/library/index.cjs"
            : "/app/node_modules/library/index.mjs",
      }),
    );
    ...
    expect(resolve).toHaveBeenCalledWith("library", "/app/page.tsx", {
      kind: "require-call",
    });
    expect(resolve).toHaveBeenCalledWith("library", "/app/page.tsx", {
  ...
  it("resolves import and require branches independently", async () => {
    const resolve = vi.fn(
      async (specifier: string, _importer: string, options: { kind: string }) => ({
        id:
          options.kind === "require-call"
  ...
  it("does not rewrite a lexically bound require function", async () => {
    const resolve = vi.fn(async () => ({ id: "/app/node_modules/library/index.cjs" }));
    const transform = createTransform(resolve);
  ...
  it("leaves relative, external, and unresolved requires unchanged", async () => {
    const resolve = vi.fn(async (specifier: string) => {
      if (specifier === "external") return { id: specifier, external: true };
      return null;
    });
    const transform = createTransform(resolve);
  ...
  it("leaves packages with the same import and require entry untouched", async () => {
    const resolve = vi.fn(async () => ({ id: "/app/node_modules/library/index.js" }));
    const transform = createTransform(resolve);
  ...
  it("loads the selected CJS source through its synthetic JavaScript identity", async () => {
    ...
    try {
      ...
      const plugin = createRequireConditionResolutionPlugin();
      ...
      const transform = transformHandler.bind({
        resolve: vi.fn(
          async (_specifier: string, _importer: string, options: { kind: string }) => ({
            id: options.kind === "require-call" ? target : `${target}.mjs`,
          }),
        ),
      } as never) as (code: string, id: string) => Promise<{ code: string }>;
