...
describe("middleware nextUrl basePath", () => {
  ...
  it("uses the decoded matcher retry for pathname cache-safety scope", () => {
    const module = { config: { matcher: "/orders/:id" } };

    expect(
      matchesMiddlewareModulePathname({
        module,
        request: new Request("https://example.test/orders%2F42"),
      }),
    ).toBe(true);
    expect(
      matchesMiddlewareModulePathname({
        module,
        request: new Request("https://example.test/invoices%2F42"),
      }),
    ).toBe(false);
  });
