...
describe("App Router Production build", () => {
  ...
  it("emits an empty RSC eligibility manifest when prerender does not run", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vinext-rsc-prewarm-empty-"));
  ...
  return <html><body>{children}</body></html>;
      ...
      fs.writeFileSync(
        ...
        "export default function Page() { return <p>plain build</p>; }",
      );
      const adapterPath = path.join(tmpDir, "cdn-adapter.mjs");
      fs.writeFileSync(
        adapterPath,
        "export default function createAdapter() { return { buildResponseHeaders() { return {}; } }; }",
      );
      ...
      const builder = await createBuilder({
        ...
        plugins: [
          vinext({
            ...
            cache: {
              cdn: {
                adapter: pathToFileURL(adapterPath).href,
