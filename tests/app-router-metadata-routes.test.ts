...
describe("metadata routes integration (App Router)", () => {
  ...
  it("honors dynamicParams = false for dynamic metadata images", async () => {
    ...
    const encodedPublicImage = await fetch(
    ...
    const doubleEncodedImage = await fetch(
    ...
    const escapedSlashImage = await fetch(
    ...
    const omittedOptionalImage = await fetch(
      `${baseUrl}/metadata-static-params-optional-missing/opengraph-image`,
    );
    expect(omittedOptionalImage.status).toBe(404);

    const page = await fetch(`${baseUrl}/metadata-static-params/unlisted-draft`);
    expect(page.status).toBe(404);

    const image = await fetch(`${baseUrl}/metadata-static-params/unlisted-draft/opengraph-image`);
    const imageBody = await image.text();
    expect(imageBody).not.toContain("UNLISTED_METADATA");
