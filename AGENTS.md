...
- `toSlash` is platform-gated (a no-op on POSIX), which is right for local-filesystem paths but wrong for strings whose separator shape is decided by the **producer**: stack frames / source-map `sources` arriving over the wire, or shape-agnostic classification of bundler ids. Those keep a deliberate unconditional `.replaceAll("\\", "/")` behind a documented local helper (see `slashifyFramePath` in `server/dev-stack-sourcemap.ts` and `slashifyModuleId` in `build/client-build-config.ts`).
