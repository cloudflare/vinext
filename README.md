...
`vite.config.*` and `wrangler.jsonc`. `vinext deploy` then validates that setup, builds the
...
`imagesOptimizer()` and add the matching Wrangler Images binding. The built-in Worker entries register
that optimizer at runtime; image optimization is not implemented or generated by `vinext deploy`.
