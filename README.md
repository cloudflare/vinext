...
| `typedRoutes`                                    | ✅  | Statically typed links: generates `.next/types/link.d.ts` typing `Link` hrefs, `useRouter`, `redirect`/`permanentRedirect`, `next/form` action, and the `Route<T>` type; App and Pages Router routes.                  |
