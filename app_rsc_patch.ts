...
export function generateRscEntry(
  ...
  // PublicFiles is used for serving public/ files before rewrites (filesystem routes).
...
${slotEntries.join(",\n")}
  ...
  return `
${publicDir ? 'import __nodeFs from "node:fs";\nimport __nodePath from "node:path";' : ""}
import {
          ...
          try {
            ...
            if (__afterStat.isFile()) {
              ...
              return __applyRouteHandlerMiddlewareContext(
                new Response(__afterContent, { status: 200, headers: { "Content-Type": __mimeTypes[__afterExt] ?? "application/octet-stream" } }),
                _mwCtx,
              );
            }
        ...
        if (__fbPublicFile.startsWith(__publicDir + __nodePath.sep)) {
          try {
            ...
            if (__fbStat.isFile()) {
              ...
              setNavigationContext(null);
              return __applyRouteHandlerMiddlewareContext(
                new Response(__fbContent, { status: 200, headers: { "Content-Type": __mimeTypes[__fbExt] ?? "application/octet-stream" } }),
                _mwCtx,
              );
            }
          } catch (__e) { if (__e?.code !== 'ENOENT') console.warn('[vinext] static file check failed:', __e); }
