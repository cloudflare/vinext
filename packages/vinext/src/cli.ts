...
/**
 * vinext compatibility CLI and migration utilities
 *
 ...
 *
 * `dev` and `build` forward to the project-local Vite CLI. New projects should
 * invoke `vite` directly; the aliases remain for migrated package scripts.
 */
...
import { runViteCli } from "./vite-cli.js";
...
function printHelp(cmd?: string) {
  if (cmd === "dev") {
    console.log(`
  vinext dev - Alias for vite dev

  Usage: vinext dev [options]

  Forwards options to the project-local Vite CLI. Prefer running \`vite dev\`
  directly. Legacy --hostname/-H flags are translated to Vite's --host flag.
`);
    return;
  }
  ...
  if (cmd === "build") {
    console.log(`
  vinext build - Alias for vite build

  Usage: vinext build [options]

  Forwards options to the project-local Vite CLI. Prefer running \`vite build\`
  directly. Legacy prerender and precompression flags remain supported; new
  projects should configure them through vinext() in vite.config.ts.
`);
    return;
  }
  ...
  Serves the output from \`vite build\`. Supports SSR, static files,
...
`);
  ...
  Options:
    ...
    --prerender          Configure vite build to pre-render all static routes
...
`);
  ...
  Commands:
    dev      Alias for vite dev
    build    Alias for vite build
  ...
  Examples:
    ...
    vite dev --port 4000               Start dev server on port 4000
  ...
  The vinext Vite plugin implements development and build behavior. The legacy
  vinext dev/build commands forward to Vite for package-script compatibility.
`);
}
...
switch (command) {
  case "dev":
    runViteCli("dev", rawArgs);
    break;

  case "build":
    runViteCli("build", rawArgs);
    break;
