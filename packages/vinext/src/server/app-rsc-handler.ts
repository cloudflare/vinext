...
async function handleAppRscRequest<TRoute extends AppRscHandlerRoute>(
  ...
  if (!filesystemRouteEligible && (actionId || isProgressiveActionRequest)) {
    ...
    for (const rewrite of options.configRewrites.afterFiles) {
      ...
      setRewrittenCleanPathname(pathnameForResolvedUrl(resolvedUrl));
      filesystemRouteEligible = true;
      actionMatch = options.matchRoute(cleanPathname, rawCleanPathname);
  ...
  // rawCleanPathname keeps dynamic route params single-decoded (#1963).
  const preActionMatch = filesystemRouteEligible
    ? options.matchRoute(cleanPathname, rawCleanPathname)
    : null;
  if (preActionMatch) {
