...
function getGeneratedParamNames(
  routeParams: AppPageParams,
  source: GenerateStaticParamsSource,
  staticParams: readonly Record<string, unknown>[],
): { routeParamNames: string[]; sourceParamNames: string[] } {
  const routeParamNames: string[] = [];
  const sourceParamNames: string[] = [];

  for (const routeParamName of Object.keys(routeParams)) {
    const sourceParamName = source.paramAliases?.[routeParamName] ?? routeParamName;
    if (staticParams.some((params) => Object.hasOwn(params, sourceParamName))) {
      routeParamNames.push(routeParamName);
      sourceParamNames.push(sourceParamName);
    }
  }

  return { routeParamNames, sourceParamNames };
}
...
export async function validateAppPageDynamicParams(
  ...
  const independentlyGeneratedParamNames = new Set<string>();
  for (const source of generateStaticParamsSources) {
    ...
    if (Array.isArray(staticParams)) {
      const generatedParamNames = getGeneratedParamNames(options.params, source, staticParams);
      for (const routeParamName of generatedParamNames.routeParamNames) {
        independentlyGeneratedParamNames.add(routeParamName);
      }
      if (
        !areStaticParamsAllowed(
          pickRouteParams(sourceParams, generatedParamNames.sourceParamNames),
          staticParams,
        )
      ) {
        options.clearRequestContext();
  ...
  if (chainedStaticParams) {
    // Next merges only the concrete keys returned by each generator. A
    // parallel branch's route shape can contain params actually generated by
    // the primary chain, so only exclude keys the parallel generator returned.
    // https://github.com/vercel/next.js/blob/v16.2.7/packages/next/src/build/static-paths/app.ts
    const chainedRequestParams = Object.fromEntries(
      Object.entries(options.params).filter(
        ([name]) => !independentlyGeneratedParamNames.has(name),
      ),
    );
    if (!areStaticParamsAllowed(chainedRequestParams, chainedStaticParams)) {
      options.clearRequestContext();
      return notFoundResponse();
    }
  }
