...
async function _handleRequest(request, __reqCtx, _mwCtx, ctx) {
  ...
  // ─── Early ISR cache check ────────────────────────────────────────────────
  // Check the ISR cache BEFORE buildPageElement / renderToReadableStream.
  // On a HIT or STALE, we can serve cached bytes immediately — no render needed.
  // This eliminates the ~3s wall-time penalty on cache HITs where the full RSC
  // render was previously started before the cache was consulted.
  //
  // Only applies in production (process.env.NODE_ENV === "production" is
  // statically replaced by Vite's define transform — this block is dead-code-
  // eliminated in dev/test mode).
  //
  // Skip for: force-dynamic, intercepting routes (RSC only — those require a
  // fresh render to assemble the intercepted slot), and RSC requests that don't
  // have a corresponding cache entry yet (first visit, pre-HTML).
  if (process.env.NODE_ENV === "production" && revalidateSeconds !== null && revalidateSeconds > 0 && !isForceDynamic) {
    const _earlyIsrKey = isrCacheKey("app", cleanPathname);
    // Skip early RSC check for intercepting routes — they render a different element tree.
    ...
    if (!_isInterceptingRsc) {
      ...
      if (_earlyCached && _earlyCached.value.value?.kind === "APP_PAGE") {
        ...
        if (isRscRequest) {
          // RSC path — serve cached rscData if present
          if (_earlyCached.value.value.rscData) {
            const _earlyRscResponseHeaders = {
              "Content-Type": "text/x-component; charset=utf-8",
              "Vary": "RSC, Accept",
              "Cache-Control": "s-maxage=" + _earlyRevalidateSecs + ", stale-while-revalidate",
              "X-Vinext-Cache": _earlyIsStale ? "STALE" : "HIT",
            };
            if (params && Object.keys(params).length > 0) {
              _earlyRscResponseHeaders["X-Vinext-Params"] = JSON.stringify(params);
            }
            if (_earlyIsStale) {
              ...
              const _earlyFrozenNavCtx = _getNavigationContext();
                 triggerBackgroundRegeneration(_earlyIsrKey, async () => {
                  ...
                  const _rgRender = performance.now();
                  ...
                  const _rgDrain = performance.now();
                  ...
                  const _regenTags2 = [cleanPathname, "_N_T_" + cleanPathname, ...getCollectedFetchTags()];
                   ...
                   const _rgEnd = performance.now();
                   console.log("[vinext:regen] path=" + cleanPathname + " type=rsc-stale build=" + Math.round(_rgBuild-_rg0) + "ms ssr_load=" + Math.round(_rgSsrLoad-_rgBuild) + "ms render=" + Math.round(_rgRender-_rgSsrLoad) + "ms html_drain=" + Math.round(_rgDrain-_rgRender) + "ms kv_set=" + Math.round(_rgEnd-_rgDrain) + "ms total=" + Math.round(_rgEnd-_rg0) + "ms");
                 }, ctx);
             }
             if (_mwCtx.headers) {
               for (const [key, value] of _mwCtx.headers) {
                 ...
                 if (lk === "set-cookie") {
                  ...
                  const existing = _earlyRscResponseHeaders["Vary"] ?? _earlyRscResponseHeaders["vary"];
                  if (existing) {
                    _earlyRscResponseHeaders["Vary"] = existing + ", " + value;
                    if (_earlyRscResponseHeaders["vary"] !== undefined) delete _earlyRscResponseHeaders["vary"];
                  } else {
                    _earlyRscResponseHeaders[key] = value;
                  }
                } else {
                  _earlyRscResponseHeaders[key] = value;
                }
            ...
            setHeadersContext(null);
            setNavigationContext(null);
            const _earlyCachedRscStream = new ReadableStream({
              start(controller) {
                controller.enqueue(new Uint8Array(_earlyCached.value.value.rscData));
                controller.close();
              }
            });
             ...
             const _earlyFrozenNavCtx = _getNavigationContext();
              triggerBackgroundRegeneration(_earlyIsrKey, async () => {
                ...
                const _regenTags2 = [cleanPathname, "_N_T_" + cleanPathname, ...getCollectedFetchTags()];
                 ...
                 setRevalidateDuration(_earlyIsrKey, revalidateSeconds);
                 const _rgEnd = performance.now();
                 console.log("[vinext:regen] path=" + cleanPathname + " type=html-stale build=" + Math.round(_rgBuild-_rg0) + "ms ssr_load=" + Math.round(_rgSsrLoad-_rgBuild) + "ms render=" + Math.round(_rgRender-_rgSsrLoad) + "ms html_drain=" + Math.round(_rgDrain-_rgRender) + "ms kv_set=" + Math.round(_rgEnd-_rgDrain) + "ms total=" + Math.round(_rgEnd-_rg0) + "ms");
               }, ctx);
           }
           setHeadersContext(null);
          setNavigationContext(null);
      ...
      if (process.env.NODE_ENV === "production" && revalidateSeconds !== null && revalidateSeconds > 0) {
        ...
        const _rssMissCacheKey = isrCacheKey("app", cleanPathname);
         triggerBackgroundRegeneration(_rssMissCacheKey, async () => {
           ...
           const _rgRscDrain = performance.now();
           ...
           // Cache with the original RSC bytes (from the first render, already served to client).
            ...
            const _rgEnd = performance.now();
            console.log("[vinext:regen] path=" + cleanPathname + " type=rsc-miss rsc_drain=" + Math.round(_rgRscDrain-_rg0) + "ms build=" + Math.round(_rgBuild-_rgRscDrain) + "ms ssr_load=" + Math.round(_rgSsrLoad-_rgBuild) + "ms render=" + Math.round(_rgRender-_rgSsrLoad) + "ms html_drain=" + Math.round(_rgDrain-_rgRender) + "ms kv_set=" + Math.round(_rgEnd-_rgDrain) + "ms total=" + Math.round(_rgEnd-_rg0) + "ms");
          }, ctx);
      }
  ...
  if (revalidateSeconds !== null && revalidateSeconds > 0) {
    if (process.env.NODE_ENV === "production") {
      const cacheKey = isrCacheKey("app", cleanPathname);
       ...
       // HIT/STALE are handled by the early ISR check above (before buildPageElement/renderToReadableStream).
       // If we reach here it is always a MISS (early check found nothing — first request for this page).
       // Tee htmlStream: stream one branch to the client immediately (first byte wins),
       // drain the other branch + rscStreamForCache to isrSet in the background.
       // This eliminates the previous ~full-render latency on MISS by not buffering the
       // entire HTML before responding.
       const [htmlStreamForClient, htmlStreamForCache] = htmlStream.tee();

       // Background cache population — runs after the response is already streaming.
       // We fire-and-forget via triggerBackgroundRegeneration so Cloudflare Workers'
       // waitUntil mechanism keeps the isolate alive until the cache write completes.
        triggerBackgroundRegeneration(cacheKey, async () => {
          const _rg0 = performance.now();
          const htmlReader = htmlStreamForCache.getReader();
          const chunks = [];
          while (true) {
          ...
          // Only cache if no dynamic API usage was detected during this render.
          // dynamicUsedDuringRender is evaluated in the outer scope before this block.
          if (!dynamicUsedDuringRender) {
           ...
           const _rgEnd = performance.now();
           console.log("[vinext:regen] path=" + cleanPathname + " type=html-miss html_drain=" + Math.round(_rgDrain-_rg0) + "ms kv_set=" + Math.round(_rgEnd-_rgDrain) + "ms total=" + Math.round(_rgEnd-_rg0) + "ms");
         }, ctx);
