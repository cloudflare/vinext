...
- \`npm run build:vinext\` builds the Cloudflare Worker output.
...
function writeTemplate(root: string, appName: string, packageManager: PackageManagerName): void {
...
export async function createVinextApp(options: CreateVinextAppOptions): Promise<void> {
  ...
  writeTemplate(root, appName, options.packageManager);
