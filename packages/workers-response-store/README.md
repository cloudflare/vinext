...
- A failed regeneration re-stores the previous response, as Next.js does. It is fresh again for its revalidation interval clamped to 3–30 seconds (3 seconds for a one-year interval, which is how vinext stores `revalidate = false`), and remains servable for at least 3 more seconds or for its original stale window, whichever is longer. Reads keep serving it until a regeneration succeeds, and a read that waited for the failed regeneration still receives its error. R2 cannot update custom metadata in place, so each re-store rewrites the response object under the same revision; if that rewrite fails, the previous object stays readable. Until the retry window ends, stale reads of it start no background regeneration, and reads past its original expiry serve it as the re-store would have instead of regenerating. R2 ETags hash an object's bytes, so each rewrite keeps the response headers in the object body alongside a value unique to that re-store; concurrent re-stores then fence each other, and R2 ends on the latest one's retry schedule. Like a Next.js re-store, it restarts the response's `Age`, moves a stored `Date` to the re-store time, and caps `max-age` and `s-maxage` in the forwarded cache-control headers and any later `Expires` at the retry window (dropping `immutable`), so Workers Cache can cache it for that window and no downstream cache keeps it past the next retry. A response with no `Expires` and no `max-age`, `no-cache` or `no-store` in its `Cache-Control` gets `max-age` set to the retry window, so a browser cannot give it a longer heuristic lifetime from `Last-Modified`. Responses whose policy forbids reuse (`private`, `no-store`, `no-cache`, or a zero lifetime with no stale window) are never re-stored, so every read still regenerates them. Responses that forbid stale serving (`s-maxage`, `must-revalidate`, `proxy-revalidate`) get the retry window but no stale window, so they hard-expire again when it ends.
