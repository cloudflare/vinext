...
- **Vite 8 renamed `esbuild` config to `oxc` with different JSX options** — our plugin sets JSX transform options conditionally based on the Vite version. Vite 7 uses `esbuild: { jsx: "automatic" }` (flat string). Vite 8 uses `oxc: { jsx: { runtime: "automatic" } }` (nested object) — the oxc transformer has a different config structure where JSX options are nested under a `jsx` key with a `runtime` property. The version is detected at runtime via `createRequire(cwd + "/package.json")` to resolve the consumer project's Vite version. Static `import { version } from "vite"` doesn't work because the plugin's compiled output resolves `vite` from its own `node_modules` (workspace root = Vite 7), not the consumer project's Vite 8.
...
- **Clean build output structure** — App Router builds output to `dist/server/` (RSC/worker entry) and `dist/server/ssr/` (SSR entry), with client assets in `dist/client/`. This is achieved by configuring `build.outDir` on each environment. The `@vitejs/plugin-rsc` handles build ordering to ensure SSR builds before RSC without RSC's `emptyOutDir` wiping the SSR output.
- **RSC as worker parent environment** — With `cloudflare({ viteEnvironment: { name: "rsc", childEnvironments: ["ssr"] } })`, the RSC environment becomes the worker entry point. SSR is a child environment that RSC can import via `import("./ssr/index.js")`.
