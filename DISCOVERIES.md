...
- **Production builds work, dev is blocked** — The `react-server` condition issue (CJS `require("react")` bypassing Vite's resolve conditions in workerd's module runner) only affects dev mode. In production builds, Rollup/Rolldown statically resolves all imports at build time with proper conditions, so `react-server` is correctly applied. The RSC bundle correctly contains `__SERVER_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE`.
