...
Every `@opentelemetry/*` package listed in the application's `package.json` is kept external to the server bundle, in addition to the packages Next.js externalizes by default. Bundling them would give the SDK's hooks and the application different copies of the API and its registered providers. Where that is not what you want, for example a deployment that bundles the server into one file and ships no `node_modules`, list the package in `transpilePackages` in `next.config` and vinext bundles it instead:
...
A package listed in `transpilePackages` is removed from the automatic externals, and the automatic `@opentelemetry/instrumentation` loader registration described above is skipped for it.
...
<table>
  ...
  <tbody>
    <tr>
      <td>
        <code>AppRender.getBodyResult</code>
      </td>
      <td>
        <code>Render.getServerSideProps</code>
      </td>
    </tr>
    <tr>
      <td>
        <code>AppRender.fetch</code>
      </td>
      <td>
        <code>Render.getStaticProps</code>
      </td>
    </tr>
    <tr>
      <td>
        <code>AppRouteRouteHandlers.runHandler</code>
      </td>
      <td>
        <code>Render.renderDocument</code>
      </td>
    </tr>
    <tr>
      <td>
        <code>ResolveMetadata.generateMetadata</code>
      </td>
      <td>
        <code>Node.runHandler</code>
      </td>
    </tr>
    <tr>
      <td>
        <code>NextNodeServer.findPageComponents</code>
      </td>
      <td>
        <code>NextNodeServer.findPageComponents</code>
      </td>
    ...
    <tr>
      <td>
        <code>NextNodeServer.getLayoutOrPageModule</code>
      </td>
      <td></td>
    </tr>
    <tr>
      <td>
        <code>NextNodeServer.createComponentTree</code>
      </td>
      <td></td>
    </tr>
    <tr>
      <td>
        <code>NextNodeServer.startResponse</code>
      </td>
      <td></td>
    </tr>
  </tbody>
