...
| RSC environment crash on dev start            | Native Node module (satori, resvg, etc.) loaded in RSC env | vinext auto-stubs these in production; in dev, ensure these are only imported in server code behind dynamic `import()`. Note: Sharp works correctly via vinext's built-in image optimization — this only applies to direct user imports of native modules. |
