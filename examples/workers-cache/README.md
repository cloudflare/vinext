...
On a route miss, vinext renders normally and completed-response admission proves that the render stayed cacheable before the adapter manually inserts it into Response Store. Cache headers never enable Workers Cache on the application Worker; they only carry the admitted response policy to the cache Worker.
