...
- **vinext dev (Cloudflare plugin): 60/73 specs pass; the 13 known gaps are
  ...
  the `history.replaceState`-beside-the-router hybrid, cacheable-404 surrogate
  headers, the `afterFiles` type-ahead rewrite, and the purge endpoint pair
  (possibly env vars not reaching the workerd runtime).
