...
```bash
pnpm dev          # Vite dev server with vinext (needs RELEASE_TAG for now, see below)
pnpm dev:next     # real Next.js dev server (ground truth; --webpack, see below)
pnpm build        # Vite build with vinext (RELEASE_TAG is required and set inline)
...
- **Vite dev with vinext (Cloudflare plugin): 65/73 specs pass; the 8 known gaps are
