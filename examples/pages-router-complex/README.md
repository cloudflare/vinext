...
- **vinext dev (Cloudflare plugin): 64/73 specs pass; the 9 known gaps are
  ...
  interactivity), the `next/image` custom-loader/`fill` path, the `/_next/image`
  403 middleware branch, the `history.replaceState`-beside-the-router hybrid,
  and the purge endpoint's unauthorized-POST response.
